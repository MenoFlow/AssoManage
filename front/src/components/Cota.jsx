...
function Cotisation({ montant }) {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:3000/api/save-json', {montant2});
                        ...
                        <tbody>
                            ...
                            return (
                                <tr key={transaction.date}>
                                    <td className={transaction.montant>=montant ? 'bg-secondary text-light' : 'bg-danger text-light'}>{transaction.expediteur}</td>
                                    <td className={transaction.montant>=montant ? 'bg-secondary text-light' : 'bg-danger text-light'}>{transaction.montant}</td>
                                    <td className={transaction.montant>=montant ? 'bg-secondary text-light' : 'bg-danger text-light'}>{(transaction.montant>=montant) ? 'payé' : 'non payé' }</td>
                                </tr>
                            )})}
                        </tbody>
                    </table>
                      ...
                      <h3>Montant</h3>              
                        ...
                        <div className="row">
                          <input
                            ...
                            onChange={(e) => setMontant2(e.target.value)}
                          />
                        </div>
